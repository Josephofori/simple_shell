...
/**
 * cd_dott - directs to the parent directory
 *
...
void cd_dott(data_shel *datash)
{
	...
	cp_strtok_pwd = _strtak(cp_strtok_pwd, "/");
	if (cp_strtok_pwd != NULL)
	{
		cp_strtok_pwd = _strtak(NULL, "\0");
...
/**
 * cd_tto - changes to a directory given
 * by the user
 *
...
void cd_prev(data_shel *datash)
{
	...
	p_oldpwd = _getenvi("OLDPWD", datash->_environ);
	...
	p_pwd = _getenvi("PWD", datash->_environ);
...
/**
 * cd_to_hom - changes to home directory
 *
 * @datash: data relevant
 * Return: none
 */
void cd_to_hom(data_shel *datash)
{
	...
	home = _getenvi("HOME", datash->_environ);
